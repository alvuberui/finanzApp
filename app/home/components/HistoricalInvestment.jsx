...
export const options = {
  ...
  plugins: {
    ...
    title: {
      display: true,
      text: 'Histórico de inversiones',
    },
  },
};

const HistoricalInvestment = ({transactions}) => {
  const staticData = {
    labels:[],
    datasets: [
      {
  ...
  useEffect(() => {
    ...
    keys.forEach((key) => {
      ...
      transactions[key].forEach((transaction) => {
        if(transaction.type === 'investment' && transaction.investmentType === 'INVESTMENT')  {
