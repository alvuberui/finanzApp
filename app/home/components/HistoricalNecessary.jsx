...
export const options = {
  ...
  plugins: {
    ...
    title: {
      display: true,
      text: 'Histórico de gastos necesarios',
    },
  },
};

const HistoricalNecessary = ({transactions}) => {
  const staticData = {
    labels:[],
    datasets: [
      {
  ...
  useEffect(() => {
    ...
    keys.forEach((key) => {
      ...
      transactions[key].forEach((transaction) => {
        if(transaction.type === 'expense' && transaction.expenseType === 'MANDATORY' ) {
...
export default HistoricalNecessary
